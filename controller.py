...
from .lib.quat_tools import euler_from_quaternion, quaternion_multiply, quaternion_from_euler
...
def move_straight(
    ...
    try:
        ...
        stop_kill(wpmover)
        print('WPMover killed')
    except SystemExit:
        print("SystemExit Done")
...
def move_turn(
    ...
    Args:
        ...
        angular_speed_limit (float, optional): Angular Velocity Limit. Defaults to 1.
    """
    wpturner = WPTurner(target, end_yaw_range,
                        PID_angular, angular_speed_limit)
    try:
        rclpy.spin(wpturner)
    except Exception and KeyboardInterrupt:
        stop_kill(wpturner)
        print("WPTurner killed")
    except SystemExit:
        print("SystemExit Done")
...
def angular(cur_pos, cur_rot, dx, dy, node: Node):
    ...
    rot_tf = quaternion_multiply(
        target_rot, (cur_rot.x, cur_rot.y, cur_rot.z, -cur_rot.w))
    _, _, rot_tf_yaw = euler_from_quaternion(
        rot_tf[0], rot_tf[1], rot_tf[2], rot_tf[3])
